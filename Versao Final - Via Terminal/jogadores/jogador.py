...
"""
Nesse arquivo temos a classe Mãe Jogador que é uma classe Abstrata e as suas Classes Filhas: Humano, Elfo, Anão e Bruxo
Escolhemos que essas iriam ser as raças que o jogador pode ser de acordo com o livro de RPG do Jogo the Witcher

Na classe mãe temos as variáveis básicas que todo jogador obrigatoriamente tem como:
Apostar, Brigar, Força, Inteligencia e Imunidade, tem o Nivel geral também, mas usamos ele como o xp do nosso personagem
Dentro de cada classe filha temos as variáveis específicas como:

-> Humano: Sedução, Persuasão e Teimosia
-> Bruxo: Reflexos Relâmpagos
-> Anão: Armadura e Dedução
-> Elfo: Sintonia da Natureza, Arcos e Artesanato
(todas as variáveis foram retiradas do livro de rgp de the witcher)

Além disso, dentro das classes temos os getters e setters para conseguir manipular as variáveis sem que utilizássemos elas diretamente
Dentro da classe Jogador temos a função aumenta_nivel_habilidade responsável por chamar a função que converte experiencia em pontos cujo 10 de experiência equivale a 1 ponto
e nessa função o usuário pode escolher qual habilidade ele quer upar, essa função é polimórfica então ela é executada nas classes filhas
e nas classes filhas, primeiro aparece a função aumenta_nivel_habilidade da classe mãe e depois a sua parte na classe filha

Ao rodar o programa, essa função recebe primeiro se o usuário quer subir de nível nas variáveis da classe mãe e depois das filhas,
o objetivo era que as duas rodassem ao mesmo tempo, mas como o usuário que escolhe qual upar, acaba que a da classe filha precisa esperar a da classe mãe executar primeiro

Além disso temos a classe atacar e curar, que são classes implementadas com as classes de Interface, de forma que, na função curar, o hp do usuário vai para o máximo
e na de atacar o usuário ataca um monstro aleatório e ao mesmo tempo recebe dano deste mesmo monstro
